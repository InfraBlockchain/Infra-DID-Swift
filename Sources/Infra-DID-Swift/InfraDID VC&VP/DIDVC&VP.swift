...
public func verifyPresentation(presentation: String, resolver: Resolvable, options: VerifyPresentationOptions = VerifyPresentationOptions()) async -> VerifiedPresentation {
  ...
  guard let verified = try? await verifyJwt(jwt: presentation, options: JwtVerifyOptions(proofPurpose: nil, audience: options.audience ?? nil, resolver: resolver)) else { return VerifiedPresentation()}
...
public func verifyPresentationPayloadOptions(payload: JwtPayload, options: VerifyPresentationOptions) async throws  {
...
//  if !(type(of: jwt) is String.Type) {
//    let a = "[a-zA-Z0-9_-]+"
//    let part = jwt.matchingStrings(regex: "^\(a).\(a).?\(a)$")
//
//    guard part[0].count != 0 else { throw JWTError(localizedDescription: "Jwt Format Error")}
//
//    return await try! normalizeJwtPresentation(input: jwt)
//  } else {
//    throw JWTError(localizedDescription: "jwt not is String")
//  }
}
...
public func normalizeJwtPresentationPayload(input: JwtPayload) -> PresentationPayload {
  ...
  let vcJwt = decodeJwt(jwt: vp.verifiableCredential[0])
  
  
  iPrint(vp)
  payload.verifiableCredential = VerifiableCredentialType.credential(normalizeCredential(input: vcJwt.payload))
  
  //guard let vc = vcJwt.payload
  if vcJwt.payload.aud != nil {
    payload.verifier = vcJwt.payload.aud!
  }
  
  payload.holder = vcJwt.payload.iss ?? ""
...
public func normalizeCredential(input: JwtPayload) -> CredentialPayload {
  ...
  //credentialPayload.proof = ["type": "JwtProof2020", "jwt": input.jwt]
